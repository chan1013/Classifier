...
class MainForm:
	def __init__(self, Form):
		...
		self.path.set("C:\\Users\\jameschan\\Source\\Repos\\Classifier\\Classifier_tf\\mnist")
		...
		#---------------------------訓練---------------------------
		#資料讀取
		self.lblPath=ttk.Label(Form,width=50, textvariable = self.path)		
		...
		self.lblDataNum=ttk.Label(Form, text="")
		#訓練次數
		...
		SpaceY=30
		#資料讀取
		self.lblPath.place(x=TopX,y=TopY)
		self.btnSelectPath.place(x=TopX,y=TopY+SpaceY) 
		self.btnLoadData.place(x=TopX,y=TopY+SpaceY*2)  
		self.lblDataNum.place(x=TopX,y=TopY+SpaceY*3)
		#訓練次數
...
#----------介面按鈕----------	
	...
	def btnStarTrain_Click(self):
		...
		self.Train(img_size=28,num_classes=10,num_steps=self.TrainTime.get(),display_step=1)
...
#----------function----------
	def LoadMNIST(self,path):
		...
		self.lblDataNum.configure(text="訓練資料筆數:{}".format(x_train.shape[0],x_test.shape[0])) 
